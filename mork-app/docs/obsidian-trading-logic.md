...
Transfers are operator commands, not autonomous planner actions. In chat, use `send <amount> <symbol-or-mint> to <wallet>`, for example `send 0.05 SOL to <wallet>`. The runtime validates the recipient, estimates USD notional against execution-authority limits, respects emergency stop and cooldown controls, retains SOL for fees, and preserves the configured BBQ minimum. SPL transfers create the recipient's associated token account when necessary.
